...
void initEditor() {
    ...
    E.filename = NULL;
    E.statusmsg[0] = '\0';
    E.statusmsg_time = 0;   //  timestamp for setting status msg
    E.dirty = 0;
...
int editorCxToRx(erow *row, int cx) {
    ...
    int j;
    for (j=0 ; j<cx; j++) {
        if (row->chars[j] == '\t') rx += (TEXTC_TAB_STOP - 1) - (rx * TEXTC_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for (cx = 0 ; cx < row->size ; cx++) {
        if (row->chars[cx] == '\t') {
            cur_rx += (TEXTC_TAB_STOP - 1) - (cur_rx * TEXTC_TAB_STOP);
        }
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    for (j = 0; j < row->size; j++) {
      if (row->chars[j] == '\t') {
        row->render[idx++] = ' ';
        while (idx % TEXTC_TAB_STOP != 0) row->render[idx++] = ' ';
      } else {
...
void editorRowInsertChar(erow *row, int at, int c) {
    ...
    row->chars[at] = c;
    editorUpdateRow(row);
    E.dirty++;      //  Could have used E.dirty = 1 because im just finna treat it as a bool but you can use this to see "how" dirty the buffer is
}
